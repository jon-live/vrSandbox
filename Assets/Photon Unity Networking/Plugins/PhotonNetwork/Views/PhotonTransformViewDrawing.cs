...
public class PhotonTransformViewDrawing : MonoBehaviour, IPunObservable
{
    ...
    void UpdatePosition()
    {
	    ...
	    int deltaX = 205;
	    int deltaZ = 230;
	    int terrainWidth = 230;
	    int terrainLength = 230;
	    float sandboxWidth = 0.4f;
	    float sandboxLength = 0.6f;
	    int mappingX = deltaX + (int) ((position.x / sandboxWidth) * terrainWidth);
	    int mappingZ = deltaZ + (int) ((position.z / sandboxLength) * terrainLength);
	    float mappingY = GameObject.Find("Terrain").GetComponent<Terrain>().terrainData.GetHeight(mappingX, mappingZ);
		
	    transform.localPosition = new Vector3(mappingX, mappingY +1.0f, mappingZ);
